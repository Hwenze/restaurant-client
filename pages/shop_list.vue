		...
		<fixed-header>
			<template slot="header">
				<view class="header">
					...
					<view class="header-txt">热门店铺列表</view>
	...
	<!-- #endif -->
		...
		<view class="activity-lists-box">
			<view class="text-card-box">
				<view class="card-item" v-for="(item, index) in lists" :key="index" @click="toUrl('/pages/menu', {shop_id: item.id})">
					<image class="item-cover" :src="item.shop_avatar" mode="scaleToFill"></image>
					<view class="item-main">
						<view class="item-content">{{item.shop_name}}</view>
						<view class="item-bottom">
							<view class="item-bottom-readnum">{{item.shop_desc}}</view>
