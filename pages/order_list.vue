<template>
	...
	<!-- #endif -->
		...
		<swiper class="list_swiper" :duration="100" @change="changeSwitch" :current="type" :indicator-dots="false" :disable-touch="false">
			<swiper-item v-for="(i, n) in swiperList" :key="n">
				<scroll-view scroll-y style="width: 100%;height: 100%;" @scrolltolower="reachBottom(type)">
					<view class="list-box" v-if="lists[n].arrList.length > 0">
						<view class="list" v-for="(item, index) in lists[n].arrList" :key="index" @click="toUrl('/pages/order_details')">
							...
							<view class="text-right f6 mb15">
								<view class="span mr15">共3件,</view>
								<view class="span"> ¥66.66元</view>
							</view>
						</view>
					</view>
...
export default {
	data() {
		return {
			...
			status: -1,
			type: 0,
			catList: [{ id: -1, name: '全部' }, { id: 0, name: '待付款' }, { id: 2, name: '已完成' }, { id: 1, name: '已取消' }],
			lists: [
				{arrList:[], current:1, through:true},
				{arrList:[], current:1, through:true},
				{arrList:[], current:1, through:true},
				{arrList:[], current:1, through:true},
			],
